...
function getInvoicesByMonth(req, res) {
  ...
  try {
    ...
    if (year && month) {
      ...
      query = `${invoiceQuery} WHERE i.date LIKE ? ORDER BY i.date DESC`;
      rows = db.prepare(query).all(`${datePrefix}%`);
    } else {
      // Return all invoices if no filter provided
      query = `${invoiceQuery} ORDER BY i.date DESC`;
...
function getCustomerInvoices(req, res) {
  ...
  try {
    ...
    const rows = db.prepare(`${invoiceQuery} WHERE i.customer_id = ? ORDER BY i.date DESC`).all(customerId);
...
function searchInvoices(req, res) {
  ...
  try {
    ...
    const query = `${invoiceQuery} ${whereClause} ORDER BY i.date DESC`;

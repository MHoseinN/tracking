<template>
  <div ref="rootRef" class="relative" :class="wrapperClass">
    <div
      ref="triggerRef"
      class="flex h-12 items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 shadow-sm transition  focus-within:ring-4 focus-within:ring-blue-100"
    ...
    <Teleport to="body">
      <div v-if="isOpen" class="fixed inset-0 z-[150]" @click="closeDropdown">
        <div
          class="fixed z-50 overflow-hidden rounded-lg border border-slate-200 bg-white shadow-[0_24px_80px_rgba(15,23,42,0.22)]"
          :style="dropdownStyle"
          @click.stop
        >
          ...
          <div class="overflow-y-auto p-2" :style="{ maxHeight: dropdownStyle.maxHeight }">
            <button
              ...
              type="button"
              class="flex w-full items-center justify-between gap-3 rounded-lg px-4 py-3 text-right text-sm font-medium transition"
            ...
            <p v-if="filteredOptions.length === 0" class="rounded-lg bg-slate-50 px-4 py-4 text-sm text-slate-500">
              {{ noResultsText }}
            </p>
          </div>
        </div>

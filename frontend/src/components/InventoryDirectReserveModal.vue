  ...
  <Teleport to="body">
    <div v-if="isOpen && unit" class="fixed inset-0 z-[120] flex items-center justify-center bg-slate-950/45 p-4">
      <div class="h-[80vh] overflow-y-auto rounded-xl bg-white shadow-md">
        <div class="border-b border-slate-100 px-4 py-4">
          <div class="flex items-center justify-between gap-4">
            <div class="flex justify-between gap-2">
              <div>
                <span class="app-button-primary !px-4">{{ unit.product_name }}</span>
              </div>
              <div>
                <span class="app-button-secondary !px-4">شماره {{ Number(unit.unit_number || 0).toLocaleString('fa-IR')
                  }}
                </span>
              </div>
            </div>
        ...
        <form class="space-y-4 px-4 py-4" @submit.prevent="handleSubmit">
          <div class="grid gap-5 grid-cols-2">
            <label class="space-y-2 col-span-2">
              <span class="text-sm font-semibold text-slate-700">نام مشتری</span>
              <SearchableLookupInput v-model="customerName" :options="customerOptions"
                no-results-text="مشتری‌ای پیدا نشد. می‌توانی نام جدید را دستی وارد کنی." placeholder="جستجو مشتری"
                @select="handleCustomerSelect" @clear="customerId = null" />
            </label>
...
const durationLabel = computed(() => {
  ...
  return diff > 0 ? `${diff.toLocaleString('fa-IR')} روزه` : 'بازه نامعتبر';
});

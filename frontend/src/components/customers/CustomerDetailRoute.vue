<template>
  <div ref="tableSectionRef" class="space-y-5">
    <Teleport to="#app-shell-actions">
      <AppButton variant="primary" block :loading="creatingList" @click="createListForCustomer">ایجاد لیست برای مشتری
      </AppButton>
    ...
    <CustomerSummaryPanel :customer="customer" :settled-amount="formatCurrency(summary.paid_total_toman)"
      ...
      :settled-percent="customerSettledPercent" :remaining-percent="customerRemainingPercent" :open="isCustomerInfoOpen"
      ...
      :changed="customerFormChanged" :saving="customerFormSaving" @toggle="isCustomerInfoOpen = !isCustomerInfoOpen"
      @update-field="updateProfileField" @update:notes="customerNotesDraft = $event" @save="saveCustomerForm" />

    <AppTablePanel>
      <template #filters>
        <AppFilterBar :expanded="filtersExpanded" collapsible columns-class="md:grid-cols-2 xl:grid-cols-4"
          advanced-columns-class="md:grid-cols-4" @update:expanded="filtersExpanded = $event">
          <label class="app-filter-field"><span class="app-filter-label">جست‌وجوی لیست</span>
            <input v-model.trim="searchQuery" class="app-filter-control" type="search"
              placeholder="جست‌وجوی شماره لیست" /></label>
          ...
          <template #advanced>
            ...
            <label class="app-filter-field"><span class="app-filter-label">جست‌وجوی مبلغ فاکتور</span>
              <input v-model.trim="invoiceAmountFilter" class="app-filter-control" type="search" inputmode="numeric"
                placeholder="مثلاً ۷۳۰٬۰۰۰" />
            </label>
          </template>
          <template #actions>
      ...
      <AppDataTable class="customer-lists-table" :column-count="9" :loading="loading" :empty="!filteredLists.length"
        min-width="100%" loading-message="در حال دریافت لیست‌های مشتری..."
        ...
        <tr v-for="(list, index) in paginatedLists" :key="list.id" class="app-table-row">
          ...
          <td>
            <AppStatusButton group="list" :status="list.status" :loading="isActionLoading(list, 'list')"
              :aria-label="`مدیریت وضعیت لیست ${displayListNumber(list)}`" @click="manageListStatus(list)" />
          </td>
          <td>
            <AppStatusButton group="invoice" :status="list.invoice_status" :loading="isActionLoading(list, 'invoice')"
              :aria-label="`مدیریت فاکتور ${displayListNumber(list)}`" @click="manageInvoice(list)" />
          </td>
          <td>
          ...
          <td class="font-black text-slate-800">{{ hasInvoice(list) ? formatCurrency(list.invoice_total_toman) : '—' }}
          </td>
          <td>
...
const filteredLists = computed(() => lists.value.filter((list) => (
  ...
  && (!invoiceAmountQuery.value || (
    hasIssuedInvoice(list)
    && normalizeNumericSearch(list.invoice_total_toman).includes(invoiceAmountQuery.value)
  ))
)));
...
function clearFilters() {
  ...
  sendStatusFilter.value = 'all'; settlementStatusFilter.value = 'all'; invoiceAmountFilter.value = '';
}
...
<style scoped>
.customer-lists-table :deep(.app-table) {
  width: 100%;
  table-layout: fixed;
}

.customer-lists-table :deep(.app-table-wrapper) {
  overflow-x: hidden;
}

.customer-lists-table :deep(th),
.customer-lists-table :deep(td) {
  padding: .65rem .4rem;
  text-align: center;
  vertical-align: middle;
}

.customer-lists-table :deep(th:nth-child(1)) {
  width: 4%;
}

.customer-lists-table :deep(th:nth-child(2)) {
  width: 13%;
}

.customer-lists-table :deep(th:nth-child(3)) {
  width: 12%;
}

.customer-lists-table :deep(th:nth-child(4)) {
  width: 12%;
}

.customer-lists-table :deep(th:nth-child(5)) {
  width: 11%;
}

.customer-lists-table :deep(th:nth-child(6)) {
  width: 11%;
}

.customer-lists-table :deep(th:nth-child(7)) {
  width: 14%;
}

.customer-lists-table :deep(th:nth-child(8)) {
  width: 15%;
}

.customer-lists-table :deep(th:nth-child(9)) {
  width: 8%;
}

.customer-lists-table :deep(.app-status-badge) {
  max-width: 100%;
...
@media (max-width: 1023px) {
  ...
  .customer-lists-table :deep(th:nth-child(3)),
  .customer-lists-table :deep(td:nth-child(3)),
  .customer-lists-table :deep(th:nth-child(6)),
  .customer-lists-table :deep(td:nth-child(6)),
  .customer-lists-table :deep(th:nth-child(8)),
  .customer-lists-table :deep(td:nth-child(8)) {
    display: none;
  }
